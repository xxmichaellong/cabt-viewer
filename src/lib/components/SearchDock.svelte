<script lang="ts">
  ...
  import RankerPanel from './RankerPanel.svelte';
  import type { DecisionView } from '../game/replay';

  type Props = {
    /** Decision readout governing the board (carried forward): MCTS search or option-ranker. */
...
<aside class="search-dock" aria-label="Decision analysis">
  ...
  {#if decisionCount > 0}
    ...
    {#if decision}
      ...
      {/if}
      {#if decision.kind === 'mcts'}
        <MctsPanel mcts={decision} {onPlayResult} />
      {:else}
        <RankerPanel ranker={decision} {onPlayResult} />
      {/if}
    {:else}
      <p class="search-dock-empty">Step forward to the agent's first decision to see its analysis.</p>
    {/if}
  {:else}
    <p class="search-dock-empty">This replay has no recorded decision analysis.</p>
  {/if}
